...
const Header = ({ sectionInView }) => {
  return (
    <div className="navbar bg-base-100 shadow-md fixed z-50" id="navbar">
      <div className="flex-1">
        <a className="btn btn-ghost normal-case text-xl" id="logo">
          <span className="logo1">WondersOf</span><span className="logo2">Morocco </span>
        </a>
      </div>
      <div className="flex-none ">
        ...
        <ul className=" px-2 lg:text-lg xl:text-xl hidden lg:menu lg:menu-horizontal">
          {sections.map((section, index) => (
            ...
            >
              <a href={section.href} id="home">{section.name}</a>
            </li>
          ))}
        </ul>
        {/* Hamburger maenu  */}
